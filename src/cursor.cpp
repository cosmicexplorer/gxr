...
const std::list<CXCursorKind> ScopeKinds{};
...
bool cursor::isValid() {
  ...
  using generic_utils::is_in_container;
  ...
  if (not is_in_container(CursorTypes, cursorType)) {
    std::cerr << "invalid cursor type" << std::endl;
    res = false;
  }
  if (not is_in_container(EntitySpecifiers, entitySpec)) {
  ...
  return res;
  // return begin_file == end_file and IsValidFilename(begin_file) and
  //      is_in_container(CursorTypes, cursorType) and
